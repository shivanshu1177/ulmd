...
for test_file in test_*.cpp; do
    if [[ -f "$test_file" ]]; then
        ...
        g++ -std=c++20 -I include "$test_file" -L "${BUILD_DIR}" -lulmd -o "${BUILD_DIR}/${test_name}" 2>/dev/null || echo "    Warning: ${test_name} build failed"
    fi
done
...
run_test "ring_control" "./${BUILD_DIR}/ringctl create --name test123 --slots 64 --elem 32 && ./${BUILD_DIR}/ringctl destroy --name test123"
